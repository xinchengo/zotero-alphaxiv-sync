...
function parseSyncPairs(): SyncPair[] {
  ...
  const result = parsed
    .map((p, index) => {
      ...
      if (typeof zoteroLibraryID !== "number") {
        ztoolkit.log(
          `parseSyncPairs: Pair ${index} has invalid zoteroLibraryID:`,
          zoteroLibraryID,
        );
        return null;
      }
      if (typeof zoteroCollectionKey !== "string") {
        ztoolkit.log(
          `parseSyncPairs: Pair ${index} has invalid zoteroCollectionKey:`,
          zoteroCollectionKey,
        );
        return null;
      }
      if (typeof alphaxivFolderId !== "string") {
        ztoolkit.log(
          `parseSyncPairs: Pair ${index} has invalid alphaxivFolderId:`,
          alphaxivFolderId,
        );
        return null;
      }
  ...
  ztoolkit.log(
    `parseSyncPairs: Returning ${result.length} valid pairs:`,
    result,
  );
  return result;
}
...
function getArxivIdFromItem(item: Zotero.Item): string | null {
  ...
  ztoolkit.log(
    `getArxivIdFromItem: Processing item ${item.id} - ${item.getField("title")}`,
  );
  ...
  for (const field of fields) {
    try {
      ...
      if (arxiv) {
        ztoolkit.log(
          `getArxivIdFromItem: Found arXiv ID ${arxiv} in field ${field}`,
        );
        return arxiv;
      }
    } catch (e) {
...
class AlphaxivClient {
  ...
  private async requestJson<T>(
    ...
    try {
      ...
      ztoolkit.log(
        `AlphaxivClient: Response status ${xhr.status} for ${method} ${path}`,
      );
      ...
      ztoolkit.log(
        `AlphaxivClient: Parsed response for ${method} ${path}:`,
        result,
      );
      return result;
    } catch (e) {
 ...
 */
async function importIntoZoteroByArxivId(
  arxivId: string,
  ...
  const formattedArxivId = arxivId.startsWith("arXiv:")
    ? arxivId
    : `arXiv:${arxivId}`;
  const bareArxivId = formattedArxivId.replace(/^arXiv:/, ""); // Remove arXiv: prefix for URL
  ztoolkit.log(
    `importIntoZoteroByArxivId: Using formatted identifier: ${formattedArxivId}, bare ID: ${bareArxivId}`,
  );
  ...
  try {
    ztoolkit.log(
      `importIntoZoteroByArxivId: Using arXiv URL approach for ${bareArxivId}`,
    );
    ...
    return new Promise((resolve, reject) => {
      Zotero.HTTP.processDocuments([arxivUrl], async function (doc) {
        try {
          ztoolkit.log(
            `importIntoZoteroByArxivId: Document loaded for ${arxivUrl}`,
          );
          ...
          ztoolkit.log(
            `importIntoZoteroByArxivId: Found ${translators.length} web translators for ${arxivUrl}`,
          );

          if (translators.length === 0) {
            throw new Error(
              `No web translators found for arXiv URL: ${arxivUrl}`,
            );
          }
          ...
          ztoolkit.log(
            `importIntoZoteroByArxivId: Successfully imported ${items.length} items via URL`,
          );
    ...
    ztoolkit.log(
      `importIntoZoteroByArxivId: Import failed for ${formattedArxivId}:`,
      e,
    );
...
async function getCollectionItemsForPair(
  ...
  ztoolkit.log(
    `getCollectionItemsForPair: Looking for collection ${pair.zoteroCollectionKey} in library ${pair.zoteroLibraryID}`,
  );
  ...
  ztoolkit.log(
    `getCollectionItemsForPair: Found collection "${collection.name}"`,
  );

  const ids = collection.getChildItems(true, false);
  ztoolkit.log(
    `getCollectionItemsForPair: Collection has ${ids.length} child items`,
  );

  const items = Zotero.Items.get(ids) as Zotero.Item[];
  const regularItems = items.filter((it) => it?.isRegularItem?.());
  ztoolkit.log(
    `getCollectionItemsForPair: ${regularItems.length} regular items found`,
  );
...
export async function syncAllPairs() {
  ...
  ztoolkit.log(
    "syncAllPairs: Retrieved folders from AlphaXiv:",
    folders.length,
  );

  for (const pair of pairs) {
    ztoolkit.log(
      `syncAllPairs: Processing pair - Zotero: ${pair.zoteroLibraryID}/${pair.zoteroCollectionKey}, AlphaXiv: ${pair.alphaxivFolderId}`,
    );

    const folder = folders.find((f) => f.id === pair.alphaxivFolderId);
    if (!folder) {
      ztoolkit.log(
        `syncAllPairs: AlphaXiv folder ${pair.alphaxivFolderId} not found in available folders`,
      );
      ztoolkit.log(
        `syncAllPairs: Available folder IDs:`,
        folders.map((f) => f.id),
      );
      continue;
    }
    ...
    ztoolkit.log(
      `syncAllPairs: Found ${alphaxivPapers.length} papers in AlphaXiv folder ${folder.name || folder.id}`,
    );
    ...
    ztoolkit.log(
      `syncAllPairs: Extracted ${alphaxivArxivIds.size} arXiv IDs from AlphaXiv:`,
      Array.from(alphaxivArxivIds),
    );

    const items = await getCollectionItemsForPair(pair);
    ztoolkit.log(
      `syncAllPairs: Found ${items.length} items in Zotero collection`,
    );
    ...
    ztoolkit.log(
      `syncAllPairs: Extracted ${zoteroArxivIds.size} arXiv IDs from Zotero:`,
      Array.from(zoteroArxivIds),
    );
    ...
    ztoolkit.log(
      `syncAllPairs: ${missingInAlphaxiv.length} papers missing in AlphaXiv:`,
      missingInAlphaxiv,
    );
    ...
    for (const arxivId of missingInAlphaxiv) {
      try {
        ...
        ztoolkit.log(
          `syncAllPairs: Got paper group ID for ${arxivId}: ${paperGroupId}`,
        );
      } catch (e) {
        ztoolkit.log(
          `syncAllPairs: Failed to get paper group ID for ${arxivId}:`,
          e,
        );
      }
    }
    ...
    if (paperGroupIdsToAdd.length > 0) {
      ztoolkit.log(
        `syncAllPairs: Adding ${paperGroupIdsToAdd.length} papers to AlphaXiv folder`,
      );
    ...
    ztoolkit.log(
      `syncAllPairs: ${missingInZotero.length} papers missing in Zotero:`,
      missingInZotero,
    );
    ...
    for (const alphaxivArxivId of missingInZotero) {
      try {
        ztoolkit.log(
          `syncAllPairs: Importing ${alphaxivArxivId} into Zotero collection`,
        );
        await importIntoZoteroByArxivId(
          alphaxivArxivId,
          pair.zoteroCollectionKey,
          pair.zoteroLibraryID,
        );
